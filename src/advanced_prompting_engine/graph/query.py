...
Authoritative source: Spec 01, Spec 12, DESIGN.md.
...
class GraphQueryLayer:
    """13 read methods over the NetworkX graph. All provenance-aware."""
    ...
    def list_branches(self) -> list[dict]:
        return [
            ...
            if self._g.nodes[n].get("type") == "branch"
        ]
    ...
    def get_construct(self, branch: str, x: int, y: int) -> dict | None:
    ...
    def get_spectrum_opposite(self, branch: str, x: int, y: int) -> dict | None:
        ...
        for _, v, data in self._g.edges(node_id, data=True):
            if data.get("relation") == SPECTRUM_OPPOSITION:
                return {"id": v, **self._g.nodes.get(v, {}), "spectrum_question": data.get("question")}
        # Check reverse direction
        for u, _, data in self._g.in_edges(node_id, data=True):
            if data.get("relation") == SPECTRUM_OPPOSITION:
                return {"id": u, **self._g.nodes.get(u, {}), "spectrum_question": data.get("question")}
        return None

    def get_edge_constructs(self, branch: str) -> list[dict]:
        """All 36 edge-classified constructs for a branch."""
        return [
            c for c in self.list_constructs(branch)
    ...
    def get_nexus(self, source_branch: str, target_branch: str) -> dict | None:
        node_id = f"nexus.{source_branch}.{target_branch}"
        if node_id in self._g.nodes:
            return {"id": node_id, **self._g.nodes[node_id]}
        return None

    def get_spoke(self, branch: str) -> list[dict]:
        """All nexus nodes originating from a branch (the spoke's 9 gems)."""
        results = []
        for n, data in self._g.nodes(data=True):
            if data.get("type") == "nexus" and data.get("source_branch") == branch:
                results.append({"id": n, **data})
        return results
